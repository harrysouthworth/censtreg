...
test_that("Left-censored regression: posterior means match generating mechanism", {
  ...
  for (i in 1:10){
    ...
    d <- data.frame(x = x, y = y, censored = as.numeric(y < th), th = unname(th))

    m[[i]] <- censtreg(y ~ x, data = d, censored = "censored", limit = d$th, silent = TRUE,
                       lognu_params = c(nu = 6, mu = log(6), s = 1))

    o[[i]] <- censtreg(y ~ x, data = d, censored = "censored", limit = d$th, silent = TRUE,
                       lognu_params = c(nu = 6, mu = log(6), s = 1),
                       method = "integrate")
...
test_that("Right-censored regression looks ok", {
  ...
  for (i in 1:10){
    ...
    m[[i]] <- censtreg(y ~ x, data = d, limit = d$th, silent = TRUE, upper = TRUE,
                       censored = "censored",
                       lognu_params = c(nu = 6, mu = log(6), s = 1))
    o[[i]] <- censtreg(y ~ x, data = d, limit = d$th, silent = TRUE, upper = TRUE,
                       censored = "censored",
                       lognu_params = c(nu = 6, mu = log(6), s = 1),
                       method = "integrate")
